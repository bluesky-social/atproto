...
function useSegmentValidator(domain: ValidDomain) {
  ...
  const validateSegment = useCallback(
    (segment: string) => {
      ...
      const validCharset = /^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/gi.test(segment)
...
export function SignUpHandleForm({
  ...
  return (
    <FormCardAsync
      ...
      append={children}
    >
      <Fieldset label="Type your desired username">
        <InputText
          ref={inputRef}
          ...
          aria-label="Type your desired username"
          title="Type your desired username"
          append={
            domains.length > 1 && (
              <select
      ...
      <ExpandTransition visible={!!segment}>
        ...
        <p className="mt-3">
          You can change to a custom domain handle after your account is set up.
        </p>
      </ExpandTransition>
    </FormCardAsync>
  )
...
function HandlePreview({ handle, ...props }: HandlePreviewProps) {
  return (
    <p {...props}>
      Your full username will be <b>@{handle}</b>
    </p>
  )
}
