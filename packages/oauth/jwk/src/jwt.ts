...
export const signedJwtSchema = z.jwt().regex(/^[^.]+\.[^.]+\.[^.]+$/, {
...
export const unsignedJwtSchema = z.jwt().regex(/^[^.]+\.[^.]+$/, {
  error: 'Invalid JWT format: expected 2 segments',
})
...
export const jwtPayloadSchema = z.looseObject({
  ...
  cnf: z
    .object({
      ...
      jku: z.url().optional(), // JWK Set URI ("kid" should also be provided)
