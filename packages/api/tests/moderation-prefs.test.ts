...
import { BskyAgent, SessionDispatcher, DEFAULT_LABEL_SETTINGS } from '../src'
...
describe('agent', () => {
  ...
  it('migrates legacy content-label prefs (no mutations)', async () => {
    const xrpcAgent = new SessionDispatcher({
      service: network.pds.url,
    })

    await xrpcAgent.createAccount({
      handle: 'user1.test',
      email: 'user1@test.com',
      password: 'password',
    })

    const agent = new BskyAgent(xrpcAgent)
