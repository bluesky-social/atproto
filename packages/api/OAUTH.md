...
communicate with Bluesky's API.
...
be made using a `BskyAgent` instance. Let's make a simple call to the API to
...
if (oauthAgent) {
  ...
  document.body.textContent = `Authenticated as ${bskyAgent.getDid()}`
