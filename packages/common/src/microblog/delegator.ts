...
import { z } from 'zod'
import { Post, Like, flattenPost, flattenLike, MicroblogClient } from './types'
import * as check from '../common/check'
import { parseAxiosError, cleanHostUrl, authCfg } from '../network/util'
import { Collection } from '../repo/types'
import * as auth from '@adxp/auth'
import * as service from '../network/service'
import MicroblogReader from './reader'
  ...
  implements MicroblogClient
{
  ...
  async maintenanceToken(): Promise<auth.Ucan> {
    if (!this.authStore) {
      throw new Error('No keypair or ucan store provided. Client is read-only.')
    }
  ...
  async relationshipToken(): Promise<auth.Ucan> {
    if (!this.authStore) {
      throw new Error('No keypair or ucan store provided. Client is read-only.')
    }
    const serverDid = await this.getOwnServerDid()
  ...
  async postToken(collection: Collection, tid: TID): Promise<auth.Ucan> {
    if (!this.authStore) {
      throw new Error('No keypair or ucan store provided. Client is read-only.')
    }
    const serverDid = await this.getOwnServerDid()
