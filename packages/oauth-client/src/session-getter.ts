        ...
        isStale: (_sessionId, { tokenSet }) => {
          return (
            ...
            tokenSet.expires_at -
              ...
              // refreshing at the exact same time, when they are started at the
              // same time.
              60e3 * Math.random()
          )
        },
        onStoreError: async (err, _sessionId, { tokenSet, dpopKey }) => {
          // If the token data cannot be stored, let's revoke it
          const server = await serverFactory.fromIssuer(tokenSet.iss, dpopKey)
          await server.revoke(tokenSet.access_token)
          throw err
        },
        deleteOnError: async (err, _sessionId) => {
          // If fetching a refresh token fails because they are no longer valid,
          // delete the session from the sessionStore.

          return isRefreshDeniedError(err)
        },
      },
    )
  }
