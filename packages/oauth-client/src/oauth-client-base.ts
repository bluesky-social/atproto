...
export abstract class OAuthClientBase {
  abstract request(pathname: string, init?: RequestInit): Promise<Response>
  ...
  static create({
    ...
    const resolver = OAuthResolver.from(resolverOptions)
    ...
    const buildServer = (serverMetadata: OAuthServerMetadata, dpopKey: Key) =>
      new OAuthServer(
        ...
        crypto,
        keyset,
        resolverOptions.fetch,
      )

    return class OAuthClient extends this {
      ...
      static async callback(params: URLSearchParams): Promise<{
        ...
        else await stateStore.del(state)
      ...
      async getAuth(refresh = Date.now() > this.tokenSet.expires_at): Promise<{
        aud: string
        header: string
      }> {
        if (refresh) {
      ...
      async request(pathname: string, init?: RequestInit): Promise<Response> {
        ...
        const request = new Request(new URL(pathname, aud), {
          ...init,
          headers,
        })
