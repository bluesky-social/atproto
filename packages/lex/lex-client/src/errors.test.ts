...
describe(XrpcResponseError, () => {
  ...
  describe('isServerError', () => {
    it('returns true for 5xx status codes', () => {
      expect(createResponseError(500, 'InternalError').isServerError).toBe(true)
      expect(createResponseError(502, 'BadGateway').isServerError).toBe(true)
      expect(createResponseError(503, 'Unavailable').isServerError).toBe(true)
    })

    it('returns false for non-5xx status codes', () => {
      expect(createResponseError(400, 'BadRequest').isServerError).toBe(false)
      expect(createResponseError(404, 'NotFound').isServerError).toBe(false)
    })
  })

  describe('isClientError', () => {
    it('returns true for 4xx status codes', () => {
      expect(createResponseError(400, 'BadRequest').isClientError).toBe(true)
      expect(createResponseError(404, 'NotFound').isClientError).toBe(true)
      expect(createResponseError(429, 'RateLimit').isClientError).toBe(true)
    })

    it('returns false for non-4xx status codes', () => {
      expect(createResponseError(500, 'Internal').isClientError).toBe(false)
      expect(createResponseError(200, 'Ok').isClientError).toBe(false)
    })
  })
