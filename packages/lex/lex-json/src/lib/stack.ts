...
export type ParentRef =
  { frame: ArrayFrame; index: number } | { frame: ObjectFrame; index: number }
...
export class Stack<TCustom = never> {
  static from<TCustom = never>(
    input: readonly unknown[] | object,
    options: Required<StackOptions>,
  ): Stack<TCustom> {
    const root = buildStackFrame(input, undefined, options)
    return new Stack<TCustom>(root, options)
  }

  private readonly stack: (StackFrame | TCustom)[]

  constructor(
    private readonly root: StackFrame,
  ...
  *[Symbol.iterator](): Generator<StackFrame | TCustom, void, unknown> {
  ...
  pushNested(input: readonly unknown[] | object, parent: ParentRef): void {
    this.push(buildStackFrame(input, parent, this.options))
  }
}
...
export function buildStackFrame(
  ...
  if (Array.isArray(input)) {
    if (input.length > options.maxContainerLength) {
    ...
    if (options.maxObjectKeyLen !== Infinity) {
      for (let index = 0; index < entries.length; index++) {
        const key = entries[index][0]
        if (!validateMaxUtf8Length(key, options.maxObjectKeyLen)) {
