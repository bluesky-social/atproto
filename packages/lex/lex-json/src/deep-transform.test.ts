...
import { type DeepTransformOptions, deepTransform } from './deep-transform.js'
...
describe(deepTransform, () => {
  ...
  describe('recursion→iteration boundary', () => {
    // Regression: a special value sitting exactly at maxRecursionDepth (the
    // recursion→iteration switch) must still be offered to the replacer. The
    // iterative walker only applies the replacer to a frame's children, never
    // its own root, so without an explicit guard the boundary object would be
    // traversed as a plain object instead of being transformed.
    it('applies the replacer to a special object at exactly maxRecursionDepth', () => {
      const special = { $special: true, keep: 'x' }
      // root(0) → child(1) → child(2) → special(3): boundary hits at depth 3
      const input = { child: { child: { child: special } } }

      const result = deepTransform(
        input,
        (obj) => {
          if ('$special' in obj) return { replaced: true }
        },
        { maxRecursionDepth: 3 },
      )

      expect(result).toStrictEqual({
        child: { child: { child: { replaced: true } } },
      })
    })

    it('still transforms a special object nested below the boundary', () => {
      const special = { $special: true }
      // The boundary object at depth 3 is a plain wrapper; the special value is
      // its child (depth 4), reached through the iterative path.
      const input = { child: { child: { child: { child: special } } } }
      ...
      expect(result).toStrictEqual({
        child: { child: { child: { child: { replaced: true } } } },
      })
    })

    it('does not offer an array at the boundary to the replacer', () => {
      const seen: object[] = []
      // The boundary object at depth 3 is an array.
      const input = { child: { child: { child: [{ leaf: 1 }] } } }
      ...
      // The array itself is never passed to the replacer; its object child is.
      expect(seen).not.toContainEqual([{ leaf: 1 }])
      expect(seen).toContainEqual({ leaf: 1 })
    })
  })

  describe('own-enumerable properties only', () => {
    // Both transform paths must iterate own-enumerable properties only, never
    // the prototype chain (the recursive path uses Object.entries, matching
    // Stack.createFrame in the iterative path). maxRecursionDepth: 0 forces the
    // iterative path; the default keeps the recursive one.
    const paths: [label: string, maxRecursionDepth: number | undefined][] = [
      ['recursive', undefined],
      ['iterative', 0],
    ]

    for (const [label, maxRecursionDepth] of paths) {
      it(`ignores inherited enumerable properties (${label} path)`, () => {
        const proto = { inherited: { $special: true } }
        ...
        // The inherited property is never pulled into the output as an own
        // property, so the inherited special value is never transformed.
        expect(Object.hasOwn(result, 'inherited')).toBe(false)
        expect(result.own).toStrictEqual({ keep: true })
      })
    }
  })
