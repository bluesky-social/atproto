...
import { Stack, type StackOptions, getCopy, isArrayFrame } from './lib/stack.js'
...
type TransformationContext = {
  ...
  /**
   * depth is used to track the current nesting level during the
...
function deepTransformRecursive(
  ...
  if (context.currentDepth > context.maxNestedLevels) {
    ...
    // Apply the replacer to the boundary object before handing off, mirroring
    // the object branch below. The iterative walker only applies the replacer
    // to a frame's children, never to its own root, so a special value (CID,
    // Uint8Array, $link, $bytes, …) sitting exactly at this depth would
    // otherwise be traversed as a plain object instead of being transformed.
    // Arrays are never offered to the replacer (see the array branch below).
  ...
  if (Array.isArray(input)) {
    ...
    // @NOTE Iterate own enumerable entries via Object.entries rather than
    // `for..in` (which would also walk the prototype chain), keeping the same
    // iteration strategy \u2014 and the same check ordering (length, then key) \u2014 as
    // the iterative path (see Stack.createFrame in ./lib/stack.js).
...
function deepTransformIterative<TContext extends TransformationContext>(
  input: object,
  replacer: (child: object, context: TContext) => unknown,
  context: TContext,
): unknown {
  const stack = Stack.from(input, {
    initialDepth: context.currentDepth,
    maxContainerLength: context.maxContainerLength,
    maxNestedLevels: context.maxNestedLevels,
    maxObjectKeyLen: context.maxObjectKeyLen,
  })
...
function transformValue<TContext extends TransformationContext>(
  value: unknown,
  replacer: (child: object, context: TContext) => unknown,
  context: TContext,
