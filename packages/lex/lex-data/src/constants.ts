/**
 * CBOR Record Size: try to keep individual records to a few dozen KBytes. If
 * you need to store more data, even text data, consider using a blob instead. A
 * reasonable maximum record size limit (MAX_CBOR_RECORD_SIZE) is 1 MiByte (1
 * mebibyte, or 1,048,576 bytes). Note that as of August 2025, the
 ...
 */
export const MAX_CBOR_RECORD_SIZE = 1024 * 1024

/**
 * JSON Record Size: the CBOR encoding is "canonical" for records, so focusing
 * only on that encoding would make sense. But sometimes it is good to also have
 * a limit on JSON encoding size. A reasonable limit (MAX_JSON_RECORD_SIZE) is 2
 * MiByte (2,097,152 bytes).
 *
 * @see {@link https://atproto.com/guides/data-validation#recommended-data-limits}
 */
export const MAX_JSON_RECORD_SIZE = 2 * 1024 * 1024

/**
 * General string length: an overall length limit on strings within a record,
 * including both those with and without Lexicon-specified string lengths.
 * Measured as bytes (UTF-8 encoded). Should try to keep these to tens of KBytes
 * at most. For an upper-bound limit (MAX_RECORD_STRING_LEN), reasonable to just
 * rely on the overall CBOR record size limit. Notably, some early
 * implementations had a 8 KByte (8192 bytes) limit.
 *
 * @see {@link https://atproto.com/guides/data-validation#recommended-data-limits}
 */
 ...
 * @see {@link https://atproto.com/guides/data-validation#recommended-data-limits}
 */
export const MAX_CBOR_CONTAINER_LEN = 128 * 1024
