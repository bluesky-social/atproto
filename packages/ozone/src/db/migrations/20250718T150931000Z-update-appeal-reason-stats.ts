...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  // to include both com.atproto.moderation.defs.ReasonAppeal and OZONE_APPEAL_REASON_TYPE
        ...
        .select([
          ...
          (eb) =>
            sql<number>`COUNT(*) FILTER(
              ...
              AND ${eb.ref('meta')} ->> 'reportType' NOT IN (${com.atproto.moderation.defs.ReasonAppeal}, ${OZONE_APPEAL_REASON_TYPE})
            )`.as('reportCount'),
          (eb) =>
            sql<number>`COUNT(*) FILTER(
              WHERE ${eb.ref('action')} = ${tools.ozone.moderation.defs.modEventReport.$type}
              AND ${eb.ref('meta')} ->> 'reportType' IN (${com.atproto.moderation.defs.ReasonAppeal}, ${OZONE_APPEAL_REASON_TYPE})
            )`.as('appealCount'),
        ])
        .groupBy('subjectDid'),
    )
    ...
    .as(
      (db as Kysely<modEvent.PartialDB>)
        ...
        .select([
          ...
          (eb) =>
            sql<number>`COUNT(*) FILTER (WHERE ${eb.ref('action')} = ${tools.ozone.moderation.defs.modEventReport.$type} AND ${eb.ref('meta')} ->> 'reportType' NOT IN (${com.atproto.moderation.defs.ReasonAppeal}, ${OZONE_APPEAL_REASON_TYPE}))`.as(
              'reportCount',
            ),
          (eb) =>
            sql<number>`COUNT(*) FILTER (WHERE ${eb.ref('action')} = ${tools.ozone.moderation.defs.modEventReport.$type} AND ${eb.ref('meta')} ->> 'reportType' IN (${com.atproto.moderation.defs.ReasonAppeal}, ${OZONE_APPEAL_REASON_TYPE}))`.as(
              'appealCount',
            ),
        ])
