...
export async function validateRequestPayload<
  ...
  A extends readonly KnownNames[] = ['json', 'urlencoded'],
>(req: IncomingMessage, schema: S, allow?: A): Promise<z.infer<S>> {
  const payload = await parseRequestPayload<A>(
    req,
    (allow || ['json', 'urlencoded']) as A,
  )
