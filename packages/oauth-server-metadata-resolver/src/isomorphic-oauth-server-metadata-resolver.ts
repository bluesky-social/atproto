  ...
  implements OAuthServerMetadataResolver
{
  ...
  async fetchServerMetadata(
    ...
    const oauthServerMetadataEndpoint = new URL(`/.well-known/${suffix}`, url)
    ...
    if (!response.ok) {
      ...
      if (suffix !== 'openid-configuration') {
        return this.fetchServerMetadata(url, 'openid-configuration', options)
      }

      throw new FetchError(
        response.status,
        `Failed to fetch ${suffix} from ${oauthServerMetadataEndpoint}`,
        { request, response },
      )
    }
