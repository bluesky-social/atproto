...
export class CachedGetter<K extends Key = string, V extends Value = Value> {
  ...
  async get(key: K, options?: GetOptions): Promise<V> {
    ...
    while ((pending = this.pending.get(key))) {
      ...
      try {
        const { isFresh, value } = await pending

        if (isFresh) return value
        if (await checkCached(value)) return value
      } catch {
    ...
    try {
      const pending: PendingItem<V> = Promise.resolve().then(async () => {
