...
export function validateInput(
  ...
  if (reqHasBody && (def.type !== 'procedure' || !def.input)) {
    if (
      ...
      // Some HTTP clients (e.g. Ruby's "net/http") will fallback the
      // content-type of every POST request to be
      // "application/x-www-form-urlencoded", even if the request body is
      // specified to be "nil". This will cause "hasBody" here to always return
      // true for POST requests generated by those clients. In order to support
      // them, we will add an exception for endpoints not expecting any input.
