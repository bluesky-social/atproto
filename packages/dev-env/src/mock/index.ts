...
export async function generateMockSetup(env: TestNetwork) {
  ...
  // create the dev-env moderator
  {
    ...
    // TODO: I am guessing if we create an account with the labeler's did here and then publish the labels
    // that should make it all work as expected? but looks like when DID is passed, we need to give it more deets
    const mod1 = await clients.loggedout.api.com.atproto.server.createAccount({
      email: 'mod-authority1@test.com',
      handle: 'mod-authority1.test',
      password: 'hunter2',
      did: env.ozone.ctx.cfg.service.did,
    })

    await agent.api.app.bsky.actor.profile.create(
      { repo: mod1.data.did },
      {
        displayName: 'Dev-env Moderation 1',
        description: `The pretend version of mod.bsky.app 1`,
      },
    )
  }
