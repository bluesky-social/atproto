...
import { Client, asNsid, asRecordKey } from '@atproto/lex'
...
describe('repo sync', () => {
  ...
  it('sync a proof of non-existence', async () => {
    const collection = asNsid(Object.keys(repoData)[0])
  ...
  describe('repo takedown', () => {
    ...
    it('does not sync a record proof unauthed', async () => {
      const collection = asNsid(Object.keys(repoData)[0])
      const rkey = asRecordKey(Object.keys(repoData[collection])[0])
