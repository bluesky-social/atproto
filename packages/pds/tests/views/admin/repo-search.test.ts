...
describe('pds admin repo search view', () => {
  ...
  it('gives relevant results', async () => {
    ...
    const sorted = result.data.repos.sort((a, b) =>
      a.handle > b.handle ? 1 : -1,
    )
    if (db.dialect === 'pg') {
      expect(forSnapshot(sorted)).toMatchInlineSnapshot(snapPg)
    } else {
      expect(forSnapshot(sorted)).toMatchInlineSnapshot(snapSqlite)
    }
  })
