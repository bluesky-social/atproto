    ...
    handler: async ({ auth, input }) => {
      ...
      if (auth.credentials.type === 'oauth') {
        auth.credentials.permissions.assertRpc({
          ...
          lxm: ids.AppBskyNotificationUnregisterPush,
        })
      }
      ...
      if (bskyAppView.did === serviceDid) {
        await bskyAppView.agent.app.bsky.notification.unregisterPush(
          input.body,
          {
            ...authHeaders,
      ...
      await agent.api.app.bsky.notification.unregisterPush(input.body, {
        ...authHeaders,
        encoding: 'application/json',
      })
    },
