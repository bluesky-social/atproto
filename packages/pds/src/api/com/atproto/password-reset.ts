...
export default function (server: Server) {
  server.com.atproto.account.requestPasswordReset(async ({ input, res }) => {
    const { services, mailer, config } = locals.get(res)
    const email = input.body.email.toLowerCase()

    const user = await services.actor.getUserByEmail(email)
  ...
  server.com.atproto.account.resetPassword(async ({ input, res }) => {
    const { services, config } = locals.get(res)
    ...
    await services.actor.updateUserPassword(user.handle, password)
  })
}
