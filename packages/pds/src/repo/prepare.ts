...
export const createWriteToOp = (write: PreparedCreate): RecordCreateOp => ({
  action: WriteOpAction.Create,
  collection: l.asNsid(write.uri.collection),
  rkey: write.uri.rkey,
  record: write.record,
})
...
export function findBlobRefs(val: LexValue): BlobRef[] {
  // Instead of a recursive function, we use a stack based generator to avoid
  // potential call stack overflows on deeply nested records. This could result
  // in infinite loops on cyclic structures, but lexicon records should never
  // be cyclic anyway.

  const blobs: BlobRef[] = []
  const stack: (LexValue | undefined)[] = [val]

  while (stack.length > 0) {
    const value = stack.pop()!
    if (!value) {
      continue // skip null/undefined/false/""/0
    } else if (Array.isArray(value)) {
      stack.push(...value)
    } else if (isPlainObject(value)) {
      if (value.$type === 'blob') {
        // Make sure that any object with a "blob" $type is strictly valid
        if (!isBlobRef(value, { strict: true })) {
          throw new InvalidRecordError(
            `Invalid blob ref at '${JSON.stringify(value)}'`,
          )
        }
        blobs.push(value)
      } else if (isLegacyBlobRef(value)) {
        throw new InvalidRecordError('Legacy blob ref format is not supported')
      } else {
        stack.push(...Object.values(value))
        // @NOTE Not checking for slurs in keys.
      }
    }
  }
