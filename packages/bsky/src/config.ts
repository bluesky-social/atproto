...
export class ServerConfig {
  ...
  static readEnv(overrides?: Partial<ServerConfigValues>) {
    ...
    const cdnUrl =
      process.env['BSKY_CDN_URL'] || process.env['BSKY_IMG_URI_ENDPOINT']
    const etcdHosts =
      overrides?.etcdHosts ?? envList(process.env['BSKY_ETCD_HOSTS'])
    // e.g. https://video.invalid/watch/%s/%s/playlist.m3u8
    const videoPlaylistUrlPattern =
      process.env['BSKY_VIDEO_PLAYLIST_URL_PATTERN']
    ...
    const suggestionsApiKey =
      process.env['BSKY_SUGGESTIONS_API_KEY'] || undefined
    ...
    const dataplaneHttpVersion =
      process.env['BSKY_DATAPLANE_HTTP_VERSION'] || '2'
