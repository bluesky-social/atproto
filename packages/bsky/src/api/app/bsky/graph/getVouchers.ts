...
import { QueryParams } from '../../../../lexicon/types/app/bsky/graph/getVouchers'
import {
  HydrationFnInput,
  PresentationFnInput,
  RulesFnInput,
...
export default function (server: Server, ctx: AppContext) {
  const getVouchers = createPipeline(
    skeleton,
    hydration,
    noBlocks,
    presentation,
  )
  server.app.bsky.graph.getVouchers({
    ...
    handler: async ({ params, auth, req }) => {
      ...
      const result = await getVouchers({ ...params, hydrateCtx }, ctx)
