...
import { LabelStreamEvent } from '../types'

export class LabelIndexer {
  started = false
  ac = new AbortController()
  queue: PQueue
  metrics: ReturnType<typeof LabelIndexer.metrics.labels>
  constructor(
    private opts: {
      redis: Redis
      stream: string
    ...
    this.metrics = LabelIndexer.metrics.labels({
  ...
  private async process(event: LabelStreamEvent) {
    const { indexingService } = this.opts
    if (event.type === 'label') {
      await indexingService.indexLabels([event.label])
    }
  }
    ...
    try {
      ...
      const event =
        typeof msg.contents.event === 'string'
          ? safeParse<LabelStreamEvent>(msg.contents.event)
          : undefined
      if (!event) {
        dataplaneLogger.error(
          { message: msg.contents },
          'skipping bad label indexer stream message',
        )
      ...
      dataplaneLogger.error(
        ...
        'failed label indexer stream message',
      )
...
function ignoreSkipErrors(event: LabelStreamEvent, err: unknown) {
  if (err instanceof InvalidRequestError) {
    return dataplaneLogger.warn(
      { err, event },
      'event skipped by label indexer',
    )
  }
  throw err
}
