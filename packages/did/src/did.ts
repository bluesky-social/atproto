...
export function checkDid<M extends string = string>(
  input: string,
  allowedMethodNames?: readonly M[],
): asserts input is Did<AsDidMethod<M>> {
  const { length } = input
  if (length > 2048) {
  ...
  if (allowedMethodNames) {
    ...
    if (!(allowedMethodNames as readonly string[]).includes(methodName)) {
      throw new InvalidDidError(input, `Unexpected method name "${methodName}"`)
    }
  }
}

export function isDid<M extends string = string>(
  input: string,
  allowedMethodNames?: readonly M[],
): input is Did<AsDidMethod<M>> {
  try {
    checkDid(input, allowedMethodNames)
    return true
  } catch (err) {
