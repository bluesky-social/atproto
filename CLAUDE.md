...
- TypeScript compilation uses the native TS7 `tsc` (`typescript@next`). There is no per-package `typescript` devDependency — it is hoisted at the root. Note TS7 has no stable programmatic API yet; tools needing one must pin TS6.
