...
- ([packages/bsky](packages/bsky)) — read-side service for `app.bsky.*` queries (timelines, profiles, feed generators, hydration pipeline, GraphQL-like view composition). Talks to PDSes via XRPC and to `bsync` via Connect-RPC (protobuf in `packages/bsky/proto`). Runtime entry point in [services/bsky](services/bsky).
  - For allowlisted `getFeed` requests, signed-in viewers check `iris:feed:enable` with their DID; guests with `X-Bsky-Device-Id` check `iris:anonymous_feed:enable` with `deviceId` (`stable_id` in event proxy). Provision the guest flag in GrowthBook with `deviceId` hashing. A false gate or guest without a stable ID uses the registered generator even when staging overlaps.
