...
- ([packages/bsky](packages/bsky)) — read-side service for `app.bsky.*` queries (timelines, profiles, feed generators, hydration pipeline, GraphQL-like view composition). Talks to PDSes via XRPC and to `bsync` via Connect-RPC (protobuf in `packages/bsky/proto`). Runtime entry point in [services/bsky](services/bsky).
  - Guest `getFeed` cutover to Iris checks `iris:feed:enable` only for allowlisted feeds with client-supplied `X-Bsky-Device-Id`; hash guest rules on `deviceId` (`stable_id` in event proxy). Flag false or missing ID uses the registered generator even when staging overlaps. Authenticated requests never evaluate this flag; staging remains independent.
