...
import SchemaStore from '../user-store/schema-store.js'
...
export class Microblog {
  programName = 'did:bksy:microblog'
  store: UserStore

  constructor(params: { userStore: UserStore }) {
    this.store = params.userStore
  }
  ...
  async runOnSchema<T>(fn: (store: SchemaStore) => Promise<T>): Promise<T> {
    return this.store.runOnSchema(this.programName, fn)
  }

  async getPost(id: TID): Promise<Post | null> {
    const postCid = await this.runOnSchema(async (program) => {
  ...
  async addPost(text: string): Promise<TID> {
    ...
    await this.runOnSchema(async (program) => {
  ...
  async listPosts(count: number, from?: TID): Promise<Post[]> {
    const entries = await this.runOnSchema(async (program) => {
  ...
  async getFollow(did: DID): Promise<Follow | null> {
    const cid = await this.runOnSchema(async (program) => {
  ...
  async isFollowing(did: DID): Promise<boolean> {
    return this.runOnSchema(async (program) => {
  ...
  async listFollows(): Promise<Follow[]> {
    const cids = await this.runOnSchema(async (program) => {
