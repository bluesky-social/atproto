...
name: lexification
description: >
  Migrate AT Protocol packages from `@atproto/api`, `@atproto/lexicon`, `@atproto/xrpc`, and `@atproto/lex-cli` to `@atproto/lex`.
  Use this skill whenever the user wants to lexify a package, replace AtpAgent with Client, migrate generated lexicon
  code to the new `lex build` output, replace `ids.XxxYyy` with namespace accessors, adopt branded string types,
  switch from `jsonStringToLex` to `lexParse`, or any refactoring that moves code from the old generated lexicon
  system to `@atproto/lex`. Also trigger when the user mentions "lexification", "lex migration", or asks about
  replacing `@atproto/api` imports in service code.
---
...
- **Prefer `@atproto/lex` imports** over `@atproto/syntax` when both export the same symbol (e.g., `DidString`, `AtUriString`).
- **Avoid `assert()` calls.** Use type guards (`isDidString()`, `isHandleString()`) with conditional logic rather than assertions.
...
1. **Package configuration** - Update `package.json` dependencies and build scripts
2. **Generated code** - Replace old `src/lexicon/` codegen with new `src/lexicons/` from `lex build`
3. **Type aliases file** - Create a centralized types file re-exporting from generated schemas
4. **App context / initialization** - Replace `AtpAgent` with `Client` from `@atproto/lex`
5. **Endpoint handler registration** - New `server.add()` pattern
6. **Type references** - Replace old codegen type imports with new namespace-based types
7. **Type strictness** - Use branded types (`DidString`, `HandleString`, `AtUriString`, `Cid`, etc.)
8. **Data utilities** - Replace old data manipulation functions
9. **XRPC client calls** - Replace `AtpAgent` API calls with `Client.xrpc()` or `xrpcSafe()`
10. **Union type discrimination** - Replace old `isX()` functions with `$matches()` / `$isTypeOf`
...
## 3. Type Aliases File (OPTIONAL)

Create a centralized `views/types.ts` (or similar) that re-exports types from generated schemas. This avoids scattering long namespace paths throughout the codebase:
...
## 5. Endpoint Handler Registration
...
Alternatively, an `satisfies` clause can be used to ensure the returned object matches the expected schema output type:
...
Use `satisfies` for inline type checking when constructing objects that should match a schema type:
...
For accessing string constants like list purposes:
...
## 7. Type Strictness (Branded Types)
...
The old `BlobRef` class from `@atproto/lexicon` is replaced by a plain data type from `@atproto/lex-data`. It is no longer a class, so `instanceof` checks are not needed:
...
- Procedure input go un a `input` sub-object (not used in this example)

### Using `Client.call()` for calls that only need the response body:
...
Do not change how tests make XRPC calls — they continue to use `AtpAgent` from `@atproto/api`.

## Common Pitfalls

1. **Import paths**: Generated files use `.js` extensions in imports (e.g., `'../lexicons/index.js'`). Always use `.js` even in TypeScript files.

2. **`$type` vs `$lxm`**: Use `$type` for record/object type strings (e.g., `app.bsky.feed.post.$type` = `'app.bsky.feed.post'`). Use `$lxm` for XRPC method identifiers used in auth checks and proxy routing.

3. **`$matches` vs `$isTypeOf`**: Use `$matches()` when data needs validation (unknown input, union discrimination). Use `$isTypeOf` when data is already validated and you just need to check the `$type` tag (e.g., filtering an array of pre-parsed objects).

4. **Branded type casts at boundaries**: Data from protobuf/data plane/Kysely returns plain strings. Cast to branded types (`as DidString`, `as AtUriString`) at the point where data enters the typed domain. Avoid `assert()` — use `as` casts at known-safe boundaries instead.

5. **`'application/json' as const`**: Handler return values need `as const` on the encoding string literal to satisfy the return type. Without it, TypeScript widens the type.

6. **Response header changes**: Old `AtpAgent` returned headers as a plain object with string indexing. New `Client`/`xrpc` returns standard `Headers` objects requiring `.get()`.

7. **`@atproto/lex-data` sub-export**: `Cid`, `parseCid`, and `BlobRef` are available from `@atproto/lex-data` for files that only need data types without the full `@atproto/lex` package. Both import paths work.
